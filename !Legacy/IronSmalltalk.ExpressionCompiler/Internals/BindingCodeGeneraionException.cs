...
namespace IronSmalltalk.ExpressionCompiler.Internals
{
    ...
    public class BindingCodeGeneraionException : SemanticCodeGenerationException
    {
        ...
        public BindingCodeGeneraionException(string message)
            : base(message)
        {
        }
        public BindingCodeGeneraionException(string message, Exception inner)
            : base(message, inner)
        {
        }
#if !SILVERLIGHT
        protected BindingCodeGeneraionException(SerializationInfo info, StreamingContext context)
            : base(info, context)
        {
        }
#endif
        ...
        private static string GetErrorDescription(NameBinding binding)
        {
            ...
            return $"{binding.Name} {((binding is IErrorBinding) ? ((IErrorBinding)binding).ErrorDescription : CodeGenerationErrors.UndefinedBinding)}";
        }
    }
}
