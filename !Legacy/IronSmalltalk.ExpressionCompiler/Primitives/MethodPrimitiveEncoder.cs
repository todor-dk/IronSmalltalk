...
namespace IronSmalltalk.ExpressionCompiler.Primitives
{
    ...
    public sealed class InvokeStaticMethodPrimitiveEncoder : MethodPrimitiveEncoder
    {
        ...
        private Expression GenerateExpression()
        {
            ...
            if (method == null)
                throw new PrimitiveInvalidMemberException(String.Format(CultureInfo.InvariantCulture, CodeGenerationErrors.MissingMethod, this.DefiningType.Name, this.MemberName));
            if (method.CallingConvention == CallingConventions.VarArgs)
                throw new PrimitiveInvalidMemberException(String.Format(CultureInfo.InvariantCulture, CodeGenerationErrors.VarArgsCallingConventionNotSupported, this.DefiningType.Name, this.MemberName));
